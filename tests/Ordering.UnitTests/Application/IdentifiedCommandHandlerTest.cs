...
public class IdentifiedCommandHandlerTest
{
    ...
    private readonly ILogger<IdentifiedCommandHandler<CreateOrderCommand, OrderSubmission>> _loggerMock;

    public IdentifiedCommandHandlerTest()
    {
        _requestManager = Substitute.For<IRequestManager>();
        _mediator = Substitute.For<IMediator>();
        _loggerMock = Substitute.For<ILogger<IdentifiedCommandHandler<CreateOrderCommand, OrderSubmission>>>();
    }
    ...
    public async Task Handler_sends_command_when_order_no_exists()
    {
        ...
        var fakeOrderCmd = new IdentifiedCommand<CreateOrderCommand, OrderSubmission>(FakeOrderRequest(), fakeGuid);
        ...
        // Assert
        Assert.IsTrue(result.OrderSubmitted);
        await _mediator.Received().Send(Arg.Any<IRequest<OrderSubmission>>(), default);
    }
